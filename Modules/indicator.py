...
def distance(lat_a,lon_a,lat_b,lon_b):
    return 6371*1e3 * np.arccos(np.sin(np.radians(lat_a))*np.sin(np.radians(lat_b)) + np.cos(np.radians(lat_a))*np.cos(np.radians(lat_b))*np.cos(np.radians(lon_b) - np.radians(lon_a)))
